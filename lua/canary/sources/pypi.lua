...
local REGISTRY_URL = "https://pypi.org/pypi"
...
function M.fetch_versions(deps, force, callback)
  ...
  for i, dep in ipairs(deps) do
    local cached = not force and cache.get(dep.name, "pypi")
    ...
    if cached then
      ...
      http.get(url, function(err, data)
        if err or not data then
          ...
          local latest = data.info and data.info.version
          results[i] = vim.tbl_extend("force", dep, { latest = latest })

          if latest then
            cache.set(dep.name, "pypi", { latest = latest, fetched = os.time() })
          end
        end
