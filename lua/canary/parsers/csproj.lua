...
function M.parse(content)
  ...
  for i, line in ipairs(lines) do
    ...
    local name, version = line:match('<PackageReference%s+Include="([^"]+)"%s+Version="([^"]+)"')

    -- Also match with attributes in different order
    if not name then
      name, version = line:match('<PackageReference%s+Version="([^"]+)"%s+Include="([^"]+)"')
      if name then
    ...
    -- Match multi-line PackageReference
    if not name then
      name = line:match('<PackageReference%s+Include="([^"]+)"')
      if name then
        ...
        if not version then
          ...
          for j = i + 1, math.min(i + 3, #lines) do
            ...
            if lines[j]:match("</PackageReference>") or lines[j]:match("/>") then
              break
            end
          end
        end
