  ...
  for i, line in ipairs(lines) do
    ...
    elseif in_dependencies then
      ...
      if brace_depth <= 0 then
        ...
        -- Parse Kotlin DSL dependency formats:
...
function M._parse_dependency_line(line)
  ...
  local config, coords = line:match('(%w+)%s*%(%s*"([^"]+)"')
  ...
  if group and name and version then
    ...
    return {
      ...
      display_name = name,
      current = version,
      dev = config_match and config_match:match("test") ~= nil or false,
    }
  end
