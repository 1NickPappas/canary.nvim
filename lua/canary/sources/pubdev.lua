...
local REGISTRY_URL = "https://pub.dev/api/packages"
...
function M.fetch_versions(deps, force, callback)
  ...
  for i, dep in ipairs(deps) do
    local cached = not force and cache.get(dep.name, "pubdev")
    ...
    if cached then
      ...
      http.get(url, function(err, data)
        if err or not data then
          ...
          if latest then
            cache.set(dep.name, "pubdev", { latest = latest, fetched = os.time() })
          end
        end
